...
iptables -t nat $ACTION PREROUTING -p tcp -i $INTERFACE -m multiport --dports 3074,27014:27050 -j DNAT --to-destination $CLIENT_IP

# Add or delete FORWARD rule to allow specified TCP ports through firewall
iptables $ACTION FORWARD -p tcp -d $CLIENT_IP -m multiport --dports 3074,27014:27050 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
